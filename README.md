...
| [OpenStreetMap places](packages/openstreetmap-places.json) | Find places inside a box around a location through OpenStreetMap's public Nominatim search, with addresses, coordinates, and mapped opening hours | Codec decode, fixed query slots, and projection of a real Nominatim response verified by EVA's JVM tests; device test pending. Installed by default |
| [Google Translate](packages/google-translate.json) | Open Live translate as a two-way conversation, one-way listening, or text only; or the camera, Transcribe, or dictation, with the languages set from the request | Package decode and intent construction checked against EVA's codec; the link, its `sl`/`tl` languages and its live-mode flag are the ones Google Translate 10.35 builds for its own Live translate shortcut; device test pending |
| [Messages](packages/messages.json) | Open an addressed, unsent SMS draft | Typed handoff verified on Pixel; does not send |
| [Mova](packages/mova.json) | Discover templates, todos and agenda rows; create and manage todos through Mova's Android intents, as the foreground fallback to Mova's own background EVA extension | All 15 capabilities were verified with EVA Debug and Mova 7.0.1 on a Pixel 11 Pro Fold. The unattended-write request follows Mova 7.1.1's source contract and has JVM coverage; Mova 7.2.1's own EVA extension completed find, create, update, complete, and delete against an org-agenda-api container at production's revision on a locked API 35 emulator, with Mova force-stopped before each call; physical-device verification is pending. |
...
| [Waze](packages/waze.json) | Start driving navigation in Waze through `waze.com/ul` links pinned to Waze: to a place's coordinates (starts the route directly), to a searched name or address, or to the Home or Work saved in Waze; optional toll and freeway avoidance | Codec decode and intent construction verified by EVA's JVM tests; device test pending. Installed by default once Waze is present (EVA after 0.40.1) |
...
| [Org memory](packages/org-memory.json) | Search, keep, learn, and forget notes in the user's shared memory served by [org-agenda-api](https://github.com/colonelpanic8/org-agenda-api) 4.7.0+, which their desktop agents also use | Decode and request/result mapping checked with EVA's package codec; endpoints covered by org-agenda-api's tests; device test pending; optional install |
| [Paseo](packages/paseo.json) | Find projects, and find workspaces by project, repository, or branch; read recent workspace or agent messages; open sessions and draft or send prompts through `paseo://` links, as the foreground fallback to Paseo's own background EVA extension. EVA 0.34.0 installs it by default once Paseo is present | Schema and codec decode verified against EVA, including `list_projects`; Paseo provider unit tests pass; device test deferred |
